...
namespace Airbnb.Repositories
{
    ...
    public class AirbnbRepository : IAirbnbRepository
    {
       ...
       // public ICollection<Airbnb> group_prediction(string grp)
      //  {

      //      return _context.Airbnb.Where(bill => bill.neighbourhood_group == grp).ToList();
     //   }

        public int group_prediction(string grp)
        {
            //var total = _context.Airbnb.Sum(i => i.price);
            List<Airbnb>items=_context.Airbnb.Where(bill => bill.neighbourhood_group == grp).ToList();
