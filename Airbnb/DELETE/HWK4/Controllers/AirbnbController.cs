...
namespace HWK4.Controllers
{
    ...
    public class AirbnbController : ControllerBase
    {
        ...
        public IActionResult EditItem(int id, [FromBody] Airbnb item)
        {
            if(item == null)
            {
                return BadRequest("There no item to update");
            }
            bool isUpdated = AirbnbRepository.getInstance().UpdateLine(id, item);
            if (isUpdated)
            {
            ...
            {
                return BadRequest("No Matching found");
            }
        }
		/// <summary>
